...
object perfStat {
  ...
  def apply(
    ...
    title = s"${u.username} - ${perfStats.txt(perfType.name)}",
    robots = false,
    moreJs = frag(
  ...
  private def glicko(perfType: PerfType, perf: Perf, percentile: Option[Double])(implicit lang: Lang): Frag = st.section(cls := "glicko")(
    h2(
      trans.perfRatingX(strong(decimal(perf.glicko.rating).toString)),
      perf.glicko.provisional option frag(
        " ", span(title := notEnoughRatedGames.txt(), cls := "details")("(", provisional(), ")")
      ),
      ". ",
  ...
  private def counter(count: lidraughts.perfStat.Count)(implicit lang: Lang): Frag =
  ...
  private def highlowSide(title: Frag => Frag, opt: Option[lidraughts.perfStat.RatingAt], color: String)(implicit lang: Lang): Frag =
    opt match {
      case Some(r) => div(
  ...
  private def fromTo(s: lidraughts.perfStat.Streak)(implicit lang: Lang): Frag =
    s.from match {
      case Some(from) => fromXToY(
  ...
  private def resultStreakSideStreak(s: lidraughts.perfStat.Streak, title: Frag => Frag, color: String)(implicit lang: Lang): Frag =
    div(cls := "streak")(
      h3(
        title(
  ...
  private def resultStreakSide(s: lidraughts.perfStat.Streaks, title: Frag, color: String)(implicit lang: Lang): Frag =
    div(
      h2(title),
  ...
  private def resultStreak(streak: lidraughts.perfStat.ResultStreak)(implicit lang: Lang): Frag =
  ...
  private def resultTable(results: lidraughts.perfStat.Results, title: Frag)(implicit lang: Lang): Frag = div(
    table(
      thead(
        tr(
  ...
  private def playStreakNbStreak(s: lidraughts.perfStat.Streak, title: Frag => Frag)(implicit lang: Lang): Frag = div(
    div(cls := "streak")(
      h3(
        title(
  ...
  private def playStreakNbStreaks(streaks: lidraughts.perfStat.Streaks)(implicit lang: Lang): Frag =
    div(cls := "split")(
      playStreakNbStreak(streaks.max, longestStreak(_)),
      playStreakNbStreak(streaks.cur, currentStreak(_))
    )

  private def playStreakNb(playStreak: lidraughts.perfStat.PlayStreak)(implicit lang: Lang): Frag =
    st.section(cls := "playStreak")(
      h2(span(title := lessThanOneHour.txt())(gamesInARow())),
      playStreakNbStreaks(playStreak.nb)
    )

  private def playStreakTimeStreak(s: lidraughts.perfStat.Streak, title: Frag => Frag)(implicit lang: Lang): Frag =
    div(
      div(cls := "streak")(
  ...
  private def playStreakTimeStreaks(streaks: lidraughts.perfStat.Streaks)(implicit lang: Lang): Frag =
    div(cls := "split")(
      playStreakTimeStreak(streaks.max, longestStreak(_)),
      playStreakTimeStreak(streaks.cur, currentStreak(_))
    )

  private def playStreakTime(playStreak: lidraughts.perfStat.PlayStreak)(implicit lang: Lang): Frag =
    st.section(cls := "playStreak")(
